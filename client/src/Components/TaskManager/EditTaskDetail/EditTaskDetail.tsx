...
import { timeInHMS, timeInSeconds } from "../../../timeConversion";
...
const EditTaskDetail: React.FC = () => {
  ...
  const [newTask, setTask] = useState<NewTaskProp>();
  const [tutorialInput, setTutorialInput] = useState<string>("");

  useEffect(() => {
    if (taskData) {
      const findTask = taskData.tasks.find((v) => v._id === id);
      if (findTask) {
        const task_length = timeInHMS(parseInt(findTask._length));
        const [h = "", m = "", s = ""] = task_length.split(":");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (newTask) {
      const sendTask = {
        ...newTask,
        _length: timeInSeconds(newTask._length!),
      };

      await axios
        .put("http://localhost:8000/task/" + id, sendTask) // Replace taskId with the actual id value
        .then((res) => {
          console.log(res);
